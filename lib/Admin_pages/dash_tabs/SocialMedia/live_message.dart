...
class _UserMessagesState extends State<LiveMessage> {
        ...
        builder: (context, AsyncSnapshot<List<LiveMessages>> snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return ListView.builder(
              itemCount: allMatches.length > 2
                  ? 2
                  : allMatches.length, // Only show the first two messages
  ...
  double _calculateMatchingPercentage(String message, List<String> words) {
    double matchingWords = words
        .where((word) => message.toLowerCase().contains(word.toLowerCase()))
        .length
        .toDouble();

    return (matchingWords / words.length) * 100; // Divide by total keywords
  }
}
